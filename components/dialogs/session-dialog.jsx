  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.activityId || !formData.title || !formData.date || !formData.location) {
      toast.error("MISSING DATA", {
        description: "All core timeframe parameters must be defined."
      })
      return
    }
    ...
    if (isEditing && session) {
      ...
      toast.success("TIMEFRAME SYNCHRONIZED", {
        description: `${formData.title.toUpperCase()} session has been updated.`
      })
    } else {
      ...
      <DialogContent className="max-w-[550px] bg-white border-none rounded-[4px] p-0 overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="bg-[#222222] px-10 py-10 relative overflow-hidden group shrink-0">
          ...
          <DialogHeader className="relative z-10 text-left">
            <p className="text-[9px] font-black text-[#F28C1B] uppercase tracking-[0.4em] mb-3">Temporal Protocol</p>
            <DialogTitle className="text-3xl font-black text-white uppercase tracking-tighter italic">
              {isEditing ? "Modify Timeframe" : "Schedule Session"}
            </DialogTitle>
            <DialogDescription className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-2 leading-loose opacity-70">
              {isEditing ? "Update operational session parameters." : "Define a new tactical session for the node."}
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          <form id="session-form" onSubmit={handleSubmit} className="p-10 space-y-8">
            <div className="grid gap-2">
              <Label htmlFor="activityId" className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic ml-1">Parent Node *</Label>
              <Select
              ...
              >
                ...
                <SelectContent className="bg-white border-slate-200 rounded-[4px] shadow-2xl">
                  {activities.map((activity) => (
                    <SelectItem key={activity.id} value={activity.id} className="font-bold text-[10px] uppercase italic">
                      {activity.title}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="grid gap-2">
              <Label htmlFor="title" className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic ml-1">Session Designation *</Label>
              <Input
                id="title"
                value={formData.title}
                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                placeholder="EX: MORNING DRILL - ALPHA"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="grid gap-2">
                <Label htmlFor="date" className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic ml-1">Operational Date *</Label>
                <Input
                  id="date"
                  type="date"
                  value={formData.date}
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="grid gap-2">
                <Label htmlFor="startTime" className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic ml-1">Initiation Time</Label>
                <Input
                  id="startTime"
                  type="time"
              ...
              <div className="grid gap-2">
                <Label htmlFor="endTime" className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic ml-1">Termination Time</Label>
                <Input
                  id="endTime"
                  type="time"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="grid gap-2">
                <Label htmlFor="instructor" className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic ml-1">Node Instructor</Label>
                <Input
                  id="instructor"
              ...
              <div className="grid gap-2">
                <Label htmlFor="maxParticipants" className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic ml-1">Max Personnel</Label>
                <Input
                  id="maxParticipants"
                  type="number"
                  min={1}
            ...
            <div className="grid gap-2">
              <Label htmlFor="notes" className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic ml-1">Tactical Notes</Label>
              <Textarea
                id="notes"
                value={formData.notes}
                onChange={(e) => setFormData({ ...formData, notes: e.target.value })}
                placeholder="ADDITIONAL OPERATIONAL INTEL..."
                rows={3}
                className="bg-[#EEEEEE] border-none rounded-[4px] py-4 text-sm font-bold text-[#222222] placeholder:text-gray-300 focus-visible:ring-1 focus-visible:ring-[#F28C1B] min-h-[100px] italic"
              />
            </div>
          </form>
        </div>
          ...
          >
            {saving ? (
              ...
              <>
                ...
                <span>{isEditing ? "Synchronize Time" : "Deploy Session"}</span>
              </>
            )}
          </button>
          <button
