...
export function SelfAssessmentDialog({ open, onOpenChange, employee }) {
    ...
    const handleSave = async () => {
        ...
        toast.success("Identity Matrix Synchronized", {
            description: "Personnel capability nodes successfully recalibrated."
        })
        onOpenChange(false)
    }
            ...
            <DialogContent className="max-w-xl bg-white border-none rounded-[4px] p-0 overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div className="bg-[#222222] px-10 py-10 relative overflow-hidden group shrink-0">
                    ...
                    <DialogHeader className="relative z-10">
                        <p className="text-[9px] font-bold text-[#F28C1B] uppercase tracking-[0.4em] mb-3">Self-Assessment Protocol</p>
                        ...
                        <DialogDescription className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-2 leading-loose opacity-70">
                            Rate your current proficiency in each capability node (0–100).
                        </DialogDescription>
                    </DialogHeader>
                </div>
                        ...
                        {assessments.length === 0 && (
                            <div className="flex flex-col items-center justify-center py-12 text-center">
                                <p className="text-[10px] text-gray-400 uppercase tracking-widest font-black italic">
                                    No capability nodes found in your profile.
                                </p>
                            </div>
                        )}
                        {assessments.map((a) => {
                                ...
                                <div key={a.skillId} className="space-y-4 p-4 rounded-xl border border-slate-50 hover:border-slate-100 transition-all bg-slate-50/30">
                                    <div className="flex items-center justify-between">
                                        <div className="space-y-1">
                                            <p className="text-sm font-bold text-slate-900 uppercase tracking-tight italic">{a.skillName}</p>
                                            <Badge variant="outline" className={cn("text-[8px] font-bold uppercase tracking-widest", getSkillLevelStyle(calculateLevel(a.score)))}>
                                                {calculateLevel(a.score)}
                                            </Badge>
                                        </div>
                                        <div className="flex items-end gap-3 text-right">
                                            {delta && (
                                                <span className={cn("text-[10px] font-black tracking-widest uppercase mb-1", delta.className)}>
                                                    {delta.value}%
                                                </span>
                                            )}
                                            <span className={`text-2xl font-black italic tracking-tighter ${getScoreColor(a.score)}`}>
                                                {a.score}%
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    />
                                    <div className="flex justify-between text-[8px] font-bold text-slate-400 uppercase tracking-[0.2em] italic">
                ...
                <DialogFooter className="px-10 py-8 border-t border-[#EEEEEE] flex gap-4 bg-[#F8FAFC]/50 shrink-0">
                    <button
                        ...
                        className="flex-1 bg-transparent border border-[#EEEEEE] hover:bg-white text-gray-400 font-black py-5 px-8 rounded-[4px] uppercase tracking-widest text-[10px] transition-all italic"
                    >
                        Abort
                    </button>
                    <button
                        onClick={handleSave}
                        disabled={saving}
                        className="flex-1 bg-[#222222] text-white font-black py-5 px-8 rounded-[4px] uppercase tracking-widest text-[10px] shadow-xl hover:bg-[#F28C1B] transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-4 italic"
                    >
                        {saving ? (
                            <>
