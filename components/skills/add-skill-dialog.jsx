    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.name || !formData.description) {
            toast.error("MISSING DATA", {
                description: "All neural parameters must be defined before initialization."
            })
            return
        }
        ...
        toast.success("NEURAL ENTRY INITIALIZED", {
            description: `Intelligence node "${formData.name.toUpperCase()}" has been integrated into the matrix.`
        })
    }
            ...
            <DialogContent className="max-w-xl bg-white border-none rounded-[4px] p-0 overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div className="bg-[#222222] px-10 py-10 relative overflow-hidden group shrink-0">
                    ...
                    <DialogHeader className="relative z-10">
                        <p className="text-[9px] font-bold text-[#F28C1B] uppercase tracking-[0.4em] mb-3">Intelligence Protocol</p>
                        ...
                        <DialogDescription className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-2 leading-loose opacity-70">
                            Define the parameters and taxonomy for a new capability entry in the global matrix.
                        </DialogDescription>
                    </DialogHeader>
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    <form id="add-skill-form" onSubmit={handleSubmit} className="px-10 py-10 space-y-8">
                        <div className="space-y-3">
                            <Label className="text-[9px] font-black text-gray-400 uppercase tracking-widest italic ml-1">Node Identification</Label>
                            <Input
                                placeholder="e.g., QUANTUM LOGIC"
                                value={formData.name}
                                onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                className="h-14 bg-[#EEEEEE] border-none rounded-[4px] px-6 text-[11px] font-black text-[#222222] uppercase tracking-widest placeholder:text-gray-300 focus-visible:ring-1 focus-visible:ring-[#F28C1B] italic"
                            />
                        </div>

                        <div className="grid grid-cols-2 gap-8">
                            <div className="space-y-3">
                                <Label className="text-[9px] font-black text-gray-400 uppercase tracking-widest italic ml-1">Taxonomy Category</Label>
                                <Select
                                    value={formData.category}
                                    onValueChange={(v) => setFormData({ ...formData, category: v })}
                                >
                                    <SelectTrigger className="h-14 bg-[#EEEEEE] border-none rounded-[4px] px-6 text-[11px] font-black text-[#222222] uppercase tracking-widest focus:ring-1 focus:ring-[#F28C1B] italic">
                            ...
                            <div className="space-y-3">
                                <Label className="text-[9px] font-black text-gray-400 uppercase tracking-widest italic ml-1">Energy Profile (Type)</Label>
                                <div className="flex gap-2">
                                    {[
                                        ...
                                        >
                                            ...
                                            <span className="text-[10px] font-black italic">{type.label}</span>
                                        </button>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="space-y-3">
                            <Label className="text-[9px] font-black text-gray-400 uppercase tracking-widest italic ml-1">Technical Briefing (Description)</Label>
                            <Textarea
                                placeholder="PROVIDE OPERATIONAL CONTEXT..."
                                value={formData.description}
                                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                                className="min-h-[120px] bg-[#EEEEEE] border-none rounded-[4px] p-6 text-[11px] font-bold text-[#222222] uppercase tracking-widest placeholder:text-gray-300 focus-visible:ring-1 focus-visible:ring-[#F28C1B] italic leading-relaxed"
                            />
                        </div>
                    </form>
                </div>
