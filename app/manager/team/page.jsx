...
export default function ManagerTeamPage() {
  ...
  const getAvgScore = (employee) => {
    ...
    return Math.round(employee.skills.reduce((s, sk) => s + (sk.score || 0), 0) / employee.skills.length)
  }
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <Avatar className="h-10 w-10 border border-slate-100 shadow-sm">
                            <AvatarImage src={employee.avatar || `https://i.pravatar.cc/150?u=${employee.id}`} className="object-cover" />
        ...
        <DialogContent className="max-w-[700px] bg-white border-none rounded-xl p-0 overflow-hidden shadow-2xl">
          <div className="bg-slate-900 px-8 py-10 relative">
            ...
            <DialogHeader className="relative z-10">
              <Badge variant="outline" className="text-[10px] font-bold text-primary border-primary/30 uppercase tracking-wider mb-2 bg-primary/5">Personnel Profile</Badge>
            ...
            <div className="flex flex-col">
              <div className="p-8 border-b border-slate-100 flex flex-col md:flex-row md:items-center gap-8 bg-slate-50/30">
                <Avatar className="h-24 w-24 rounded-2xl border-2 border-white shadow-xl bg-white">
                  <AvatarImage src={selectedEmployee.avatar || `https://i.pravatar.cc/150?u=${selectedEmployee.id}`} className="object-cover" />
                  <AvatarFallback className="text-2xl font-bold text-slate-400">{selectedEmployee.name[0]}</AvatarFallback>
                </Avatar>
                <div className="flex-1">
                  <div className="flex flex-wrap gap-2 mb-3">
                    <Badge variant="secondary" className="text-[10px] font-bold px-2 py-0.5 border-slate-200 text-slate-600 uppercase tracking-wider">{selectedEmployee.department}</Badge>
                    <Badge variant="secondary" className="text-[10px] font-bold px-2 py-0.5 border-slate-200 text-slate-600 uppercase tracking-wider">ID: {selectedEmployee.matricule}</Badge>
                  </div>
                ...
                <div className="p-8 max-h-[400px] overflow-y-auto">
                  <TabsContent value="skills" className="mt-0 outline-none space-y-4">
                    {selectedEmployee.skills.map((skill) => (
                      <div key={skill.skillId} className="flex items-center justify-between p-4 rounded-xl border border-slate-100 bg-white hover:border-primary/20 transition-all shadow-sm">
                        <div className="flex items-center gap-4">
                          <div className={cn(
                            "w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold text-xs shadow-sm",
                            skill.level === "expert" ? "bg-slate-900" : skill.level === "high" ? "bg-primary" : "bg-slate-300"
                          )}>
                            {skill.level[0].toUpperCase()}
                          </div>
                          <div>
                            <p className="text-sm font-bold text-slate-900">{skill.skill.name}</p>
                            <p className="text-xs text-slate-400">{getSkillTypeLabel(skill.skill.type)}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-6">
                          <div className="text-right shrink-0">
                            <p className="text-base font-bold text-slate-900 leading-none">{skill.score}%</p>
                            <div className="flex items-center justify-end gap-1 mt-1">
                              {getProgressionIcon(skill.progression)}
                              <span className={cn(
                                "text-[10px] font-bold",
                                skill.progression > 0 ? "text-emerald-500" : skill.progression < 0 ? "text-rose-500" : "text-slate-400"
                              )}>
                                {skill.progression > 0 ? "+" : ""}{skill.progression}%
                              </span>
                            </div>
                          </div>
                          <div className="w-20 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div className="h-full bg-primary/70 transition-all duration-700" style={{ width: `${skill.score}%` }} />
                          </div>
                        </div>
                      </div>
                    ))}
                  ...
                  <TabsContent value="info" className="mt-0 outline-none space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                        ...
                        <p className="text-sm font-semibold text-slate-700">{new Date(selectedEmployee.date_embauche).toLocaleDateString()}</p>
                      </div>
                      <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                        <div className="flex items-center gap-2 mb-2">
                          <Briefcase className="w-4 h-4 text-primary" />
                          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Contract Type</p>
                        </div>
                    ...
                    <div className="p-6 rounded-xl border border-slate-100 bg-slate-50/50">
                      <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Role description</p>
